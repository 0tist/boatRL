...
class GeneticAlgorithm:
    ...
    def mutation(self, chromosome):
        ...
        if random.random() < self.config['mutation_probability'] / 3:
            chromosome.kp = chromosome.kp + np.random.random((1, 3))/max_gain_value
        if chromosome.kp.any() < 0:
            chromosome.kp = random.random((1, 3)) * max_gain_value

        elif random.random() < self.config['mutation_probability'] * 2/3:
            chromosome.ki = chromosome.ki + np.random.random((1, 3))/max_gain_value
        if chromosome.ki.any() < 0:
            chromosome.ki = random.random((1, 3)) * max_gain_value

        elif random.random() < self.config['mutation_probability']:
            chromosome.kd = chromosome.kd + np.random.random((1, 3))/max_gain_value
        if chromosome.kd.any() < 0:
            chromosome.kd = random.random((1, 3)) * max_gain_value
