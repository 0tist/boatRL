...
import numpy as np
import math
from python_vehicle_simulator.lib.control import DPpolePlacement, SimplePID
...
class supply:
    ...
    def __init__(
        ...
        elif controlSystem == "simpleControl":
            self.controlDescription = (
                "Simple PID control (x_d, y_d, psi_d) = ("
                + str(r_x)
                + " m, "
                + str(r_y)
                + " m, "
    ...
    def simpleControl(self, eta, nu, sampleTime):
        """
    ...
    def RL_reward(self):
        k = 1
        EPS = 0.00005
        cartesian_sum = np.linalg.norm(self.e_int[:2])
        yaw_error = k / cartesian_sum * self.e_int[-1]
