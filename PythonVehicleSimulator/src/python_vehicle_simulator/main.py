...
x, y = 0, 0
yaw = 0
curr_speed, curr_dir = 4, 0
control_sys = 'DPcontrol'
...
match no:                       #  the match statement requires Python >= 3.10
    ...
    case '7': vehicle = supply(control_sys, y, x, yaw ,curr_speed, curr_dir) # (controlsystem, desired y coordinate, desired x coordinate, yaw, current speed, current direction)
...
def main():    
    ...
    if int(no) == 7:
        [simTime, simData], rl_reward, current_states = simulate(N, sampleTime, vehicle)
    ...
    if int(no) == 7:
        plotRLreward(rl_reward, 4)
        plotCurrentStates(simTime, current_states, 5) 
