...
def plotCurrentStates(simTime, current_states, figNo):
    ...
    plt.subplot(2, 2, 1)
    plt.plot(t, [x[0][0] for x in current_states])
    plt.legend(["current speed"], fontsize=legendSize)
    plt.grid()
    
    plt.subplot(2, 2, 2)
    plt.plot(t, [x[0][1] for x in current_states])
    plt.legend(['current speed noise'], fontsize=legendSize)
    plt.grid()
    
    plt.subplot(2, 2, 3)
    plt.plot(t, [x[1][0] for x in current_states])
    plt.legend(["current angle"], fontsize=legendSize)
    plt.grid()
    
    plt.subplot(2, 2, 4)
    plt.plot(t, [x[1][1] for x in current_states])
    plt.legend(['current angle noise'], fontsize=legendSize)
    plt.grid()
...
def plotVehicleStates(simTime, simData, figNo, reward=None):
    ...
    plt.subplot(4, 2, 2)
    plt.plot(t, y)
    plt.legend(["North(Y)"], fontsize=legendSize)
    plt.grid()
    ...
    plt.subplot(4, 2, 3)
    plt.plot(t, x)
    plt.legend(["East(X)"], fontsize=legendSize)
    plt.grid()
    ...
    plt.subplot(4, 2, 6)
    plt.plot(t, phi, t, theta, t, psi)
    plt.legend(["Roll angle (deg)", "Pitch angle (deg)", "Yaw angle(deg)"], fontsize=legendSize)
    plt.grid()
...
def plotRLreward(rl_reward, figNo):
    
    plt.figure(
        figNo, figsize=(cm2inch(figSize1[0]), cm2inch(figSize1[1])), dpi=dpiValue
    )
    plt.subplot(1, 1, 1)
    plt.plot(range(len(rl_reward)), rl_reward, '-o')
    plt.legend(["RL reward"])
    plt.grid()
...
def plot3D(simData,numDataPoints,FPS,filename,figNo):
    ...
    initial_pt = [dataSet[0][0], dataSet[1][0], dataSet[2][0]]
    final_pt = [dataSet[0][-1], dataSet[1][-1], dataSet[2][-1]]
    initial_dot = plt.scatter(*initial_pt, c='g', marker='*')
    final_dot = plt.scatter(*final_pt, c='r', marker='*')
    ...
    #plotting intial and finla point
    ax.scatter(*initial_pt, c='g', marker='*')
    ax.scatter(*final_pt, c='r', marker='*')
