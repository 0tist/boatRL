...
class DeepLagrangianNetwork(nn.Module):
    ...
    def __init__(self, q_dim, n_width=128, n_depth=1, device="cpu", **kwargs):
        super().__init__()
        hidden_dim = n_width
        self.q_dim = q_dim
        self.device = device
    ...
    def _dyn_model(self, q, q_dot):
        ...
        return H.squeeze(), c.squeeze(), g.squeeze()

    def inv_dyn(self, q, qd, qdd):        
        H, c, g = self._dyn_model(q, qd)
        # Compute the Torque using the inverse model:
        H_qdd = torch.matmul(H, qdd.view(-1, self.q_dim, 1)).view(-1, self.q_dim)
        tau_pred = H_qdd + c + g        
        return tau_pred
    ...
    def for_dyn(self, q, qd, tau,):
        ...
        qdd_pred = torch.matmul(invH, (tau - c - g).view(-1, self.q_dim, 1)).view(-1, self.q_dim)
        return qdd_pred

    def forward(self, q, qd, qdd, tau=None, get_qdd_pred=False):
        tau_pred = self.inv_dyn(q, qd, qdd)
        qdd_pred = None
        if get_qdd_pred:
            # Compute Acceleration, e.g., forward model:
            self.for_dyn(q, qd, tau)
        return tau_pred, qdd_pred
